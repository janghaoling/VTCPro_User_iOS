...
class PresenterProcessor {
    ...
    func CheckDrivingZone(data : Data)->checkDrivingZone? {
        return data.getDecodedObject(from: checkDrivingZone.self)
    }
    
    
    //  checkPoiPriceLogic
    
    func CheckPoiPriceLogic(data : Data)->checkPoiPriceLogic? {
        return data.getDecodedObject(from: checkPoiPriceLogic.self)
    }
    
 
    
    //  GetServicePOIDisatnceInfo
    
    func GetServicePOIDisatnceInfo(data : Data)->[Service] {
        return data.getDecodedObject(from: [Service].self) ?? []
    }
    ...
    func GetServiceType(data : Data)->[Service]
    {
        return data.getDecodedObject(from: [Service].self) ?? []
    }
    ...
    func request2Array (data : Data)->[Request2] {
        return data.getDecodedObject(from: [Request2].self) ?? []
    }

...
class SideBarTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.initialLoads()
        NotificationCenter.default.addObserver(self, selector: #selector(gotAds), name: NSNotification.Name("NOTIFICATION.NAME_GOTADS"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(failedAds), name: NSNotification.Name("NOTIFICATION.NAME_FAILEDADS"), object: nil)
    }
    ...
    deinit {
        NotificationCenter.default.removeObserver(NSNotification.Name("NOTIFICATION.NAME_GOTADS"))
        NotificationCenter.default.removeObserver(NSNotification.Name("NOTIFICATION.NAME_FAILEDADS"))
    }
}
...
extension SideBarTableViewController {
    ...
    private func setDesigns () {
        ...
        Common.setFont(to: lbWallet, size: 14)
    }
...
//        self.labelName.text = String.removeNil(User.main.firstName)+" "+String.removeNil(User.main.lastName)
        let userType = String.removeNil(User.main.userType)
        if userType == Constants.string.usertype_company {
            self.labelName.text = String.removeNil(User.main.companyName)
        }
        else {
            self.labelName.text = String.removeNil(User.main.firstName)+" "+String.removeNil(User.main.lastName)
        }
        self.labelEmail.text = User.main.email
        self.lbWallet.text = "\(Formatter.shared.appPriceFormat(string: "\(Float.removeNil(User.main.wallet_balance))"))"
        self.lbWallet.textColor = Float.removeNil(User.main.wallet_balance) < 0 ? .red : .black
    ...
    @objc private func gotAds() {
        // 129 , 219
        var rect = self.viewOver.frame
        rect.size.height = 219
        self.viewOver.frame = rect
        self.view.setNeedsLayout()
        self.tableView.reloadData()
        adsContainer.isHidden = false
    }
    
    @objc private func failedAds() {
        var rect = self.viewOver.frame
        rect.size.height = 129
        self.viewOver.frame = rect
        self.view.setNeedsLayout()
        self.tableView.reloadData()
        adsContainer.isHidden = true
    }

...
class RideStatusView: UIView {
    ...
    @IBAction func showProfileProviderPicture(_ sender: Any) {
        let vc = FullScreenSlideshowViewController()
        vc.inputs = [
            AlamofireSource(urlString: self.providerAvatarUrl)!
        ]
...
extension RideStatusView {
    ...
    func set(values : Request) {
        ...
        self.providerAvatarUrl = Common.getImageUrl(for: values.provider?.avatar)
    ...
    @IBAction private func callAction() {
        ...
        //Common.call(to: "\(request?.provider?.country_code ?? "")\(request?.provider?.mobile ?? "")" )
        
//        let vc = VoiceChatViewController()
//        vc.set(connected: false, user:(self.request?.provider!)!, requestId: User.main.id!)
...
//        navigation.setNavigationBarHidden(true, animated: false)
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            chatObject.user_id = self.request?.provider?.id
            chatObject.message = User.main.firstName! + " wants to call you!  Tap here to answer"
            parentViewController!.presenter?.post(api: .chatPush, data: chatObject.toData())
        }
        
        if let VoicePage = parentViewController?.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.VoiceChatViewController) as? VoiceChatViewController {
            VoicePage.set(connected: true, user: (self.request?.provider!)!, requestId: User.main.id!)

...
extension GMSMapView {
    ...
    func drawPolygonStop(from source: CLLocationCoordinate2D, to destination: CLLocationCoordinate2D, stop Secondstop: CLLocationCoordinate2D)
    {
        self.getGoogleResponse(between: source, to: destination) { (mapPath) in
            if let points = mapPath.routes?.first?.overview_polyline?.points
            {
                
                self.getGoogleResponse(between: destination, to: Secondstop)
                { (mapPath) in
                    
                    if let points1 = mapPath.routes?.first?.overview_polyline?.points
                    {
                        self.drawPath(with: points1)
                        gmsPath = GMSPath.init(fromEncodedPath: points1)!

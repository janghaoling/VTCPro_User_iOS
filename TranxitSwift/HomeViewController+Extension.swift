...
extension HomeViewController {
    ...
    func showRideNowView(with source : [Service]) {
        ...
        if self.rideNowView == nil {
            ...
            self.rideNowView?.onClickService = { [weak self] service in
                ...
                self.sourceMarker.snippet = service?.time
                self.mapViewHelper?.mapView?.selectedMarker = (service?.time) == nil ? nil : self.sourceMarker
    ...
    func showEstimationView(with service : Service)
    {
        ...
        if self.estimationFareView == nil {
            ...
            let isWalletAvailable = User.main.wallet_balance != 0 || User.main.userType == Constants.string.usertype_company
...
//                })
                self?.scheduleRecurrentPickerView(on: { (date, recurrent) in

                    let srnv = Bundle.main.loadNibNamed(XIB.Names.ScheduleRideNoteView, owner: self, options: [:])?.first as? ScheduleRideNoteView
                    srnv?.frame = CGRect(x: 0, y: 0, width: (self?.view.frame.width)!, height: (self?.view.frame.height)!)
                    srnv?.show(with: .bottom, completion: nil)
                    self?.view.addSubview(srnv!)
                    srnv?.onClickDismiss = { note, isDimiss in
                        srnv?.removeFromSuperview();
                        self?.createRequest(for: service, isScheduled: true, scheduleDate: date,recurrent:recurrent, note:note, cardEntity: selectedPaymentDetail, paymentType: paymentType)
                    }
            ...
            self.estimationFareView?.rideNowAction = { [weak self] service in
                ...
                srnv?.onClickDismiss = { note, isDimiss in
                    srnv?.removeFromSuperview();
                    self?.createRequest(for: service, isScheduled: false, scheduleDate: nil, recurrent:nil, note:note, cardEntity: selectedPaymentDetail, paymentType: paymentType)
                }
...
//                print("Called",#function)
                ...
                else
                {
                    ...
                    {
                        self.istap = true
                        if self.invoiceView?.paymentType == .WALLET && User.main.allow_negative == 1
                        {
...
//                print("88888", self.currentRequestId)
//                self.isInvoiceShowed = false
                var rate = Rate()
...
//                        }
    ...
    func handle(request : Request) {
        ...
        guard let status = request.status, request.id != nil
            else {
                return
...
//        print("jkljkl", status)
        ...
        case .completed:
            ...
            if request.payment_mode == .CARD || request.payment_mode == .WALLET
            {
...
//                    }
                ...
                if request.use_wallet == 1 {
                    if request.paid == 0 && User.main.userType != Constants.string.usertype_company {
                        self.showInvoiceView(with: request)
                    } else {
                        if isInvoiceShowed  {
                            self.showRatingView(with: request)
    ...
    func shareRide() {
        if let currentLocation  = currentLocation.value {
            ...
            self.share(items: [#imageLiteral(resourceName: "logo-1"), message])
        }
    }
    ...
    private func addFloatingButton(with padding : CGFloat, to provider : Provider?) {
        ...
        if provider != nil {
            ...
            floaty.addItem(icon: #imageLiteral(resourceName: "call").resizeImage(newWidth: 25)) { (_) in
                //let phone = "\(provider!.country_code ?? "")\(provider!.mobile ?? "")"
                //Common.call(to: phone)
                DispatchQueue.global(qos: .background).async {
                    var chatObject = ChatPush()
                    chatObject.sender = .provider
                    chatObject.user_id = provider?.id
                    chatObject.message = User.main.firstName! + " wants to call you!  Tap here to answer"
                    self.presenter?.post(api: .chatPush, data: chatObject.toData())
                }
                if let vc = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.VoiceChatViewController) as? VoiceChatViewController {
                    vc.set(connected: false, user: provider!, requestId: User.main.id!)
                    let navigation = UINavigationController(rootViewController: vc)

...
extension PassbookWalletTableViewCell {
    ...
    func set(value : WalletTransaction) {
        ...
        self.labelAmount.text = Constants.string.amount.localize() + " : " + Formatter.shared.appPriceFormat(string:"\(value.amount ?? 0)")
//        self.labelAmount.text = Constants.string.amount.localize()+" : "+"\(User.main.currency ?? .Empty) \(Formatter.shared.limit(string: "\(value.amount ?? 0)", maximumDecimal: 2))"
        ...
        self.labelBalance.text = Constants.string.balance.localize()+" : "+Formatter.shared.appPriceFormat(string: "\(value.close_balance ?? 0)")

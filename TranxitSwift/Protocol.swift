...
extension PostViewProtocol {
    ...
    func CheckDrivingZone(api : Base, data : checkDrivingZone?) { return }
    
    func CheckPoiPriceLogic(api : Base, data : checkPoiPriceLogic?) { return }
    
    func GetServicePOIDisatnceInfo(api : Base, data : [Service]) { return }
    
     func GetServiceType(api : Base, data : [Service]) { return }
    
    
    func getRequest(api : Base, data : Request?) { return }
    func getRequestArray(api : Base, data : [Request]) { return }
    func getRequest2Array(api : Base, data : [Request2]) { return }

...
class User : NSObject, NSCoding, JSONSerializable {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        let userType = aDecoder.decodeObject(forKey: Keys.list.userType) as? String
        let companyName = aDecoder.decodeObject(forKey: Keys.list.companyName) as? String
        let wallet_limit = aDecoder.decodeObject(forKey: Keys.list.wallet_limit) as? Double
        let allow_negative = aDecoder.decodeObject(forKey: Keys.list.allow_negative) as? Int
        
        self.init(id: id, accessToken : accessToken, firstName : firstName, lastName : lastName, mobile : mobile, country_code: country_code, email: email, currency : currency, picture : picture, refreshToken : refreshToken, walletBalance : walletBalance, sos : sos,loginType : loginType, dispatcherNumber : dispatcherNumber, isBrainTreeAllowed : isBrainTreeAllowed, isPayumoneyAllowed : isPayumoneyAllowed, isPaytmAllowed : isPaytmAllowed, isCardAllowed : isCardAllowed, isCashAllowed : isCashAllowed, measurement : measurement, stripeKey : stripeKey,referral_count:referral_count,referral_unique_id:referral_unique_id,referral_total_text:referral_total_text,referral_text:referral_text,otp:otp,ride_otp:ride_otp, qrcode_url: qrcode_url, userType:userType, companyName: companyName, wallet_limit: wallet_limit, allow_negative: allow_negative)
    ...
    func encode(with aCoder: NSCoder) {
        ...
        aCoder.encode(self.userType, forKey: Keys.list.userType)
        aCoder.encode(self.companyName, forKey: Keys.list.companyName)
        aCoder.encode(self.wallet_limit, forKey: Keys.list.wallet_limit)
        aCoder.encode(self.allow_negative, forKey: Keys.list.allow_negative)
    }

...
class InvoiceView: UIView {
    ...
    @IBOutlet weak var labelPeakString: UILabel!
    @IBOutlet weak var labelPeak: UILabel!
    ...
    @IBOutlet weak var viewPeak: UIView!
    //MARK:- Local Variable
    ...
    var paymentType : PaymentType = .NONE { // Check Payment Type
        didSet {
            if paymentType != oldValue {
                ...
                if User.main.userType == Constants.string.usertype_company && request?.use_wallet == 1 {
                    paymentString = Constants.string.wallet.localize()
                }
                ...
                self.labelPaymentType.startLocation = ((text.count)-(paymentString.count))
                self.labelPaymentType.length = paymentString.count
...
extension InvoiceView {
    ...
    private func setDesign() {
        ...
        Common.setFont(to: labelPeak)
        Common.setFont(to: labelPeakString)
    ...
    private func localize() {
        ...
        self.labelPeakString.text = Constants.string.peak.localize()
    ...
    func set(request : Request) {
        ...
        print(self.request?.total_price ?? "")
        ...
        else if (isShowingRecipt || request.paid == 0 || request.payment_mode == .CARD)
        {
                self.buttonPayNow.setTitle(Constants.string.Done.localize(), for: .normal)
        }
        else {
...
//            label.text = "\(String.removeNil(User.main.currency)) \(Formatter.shared.limit(string: "\(Float.removeNil(amount))", maximumDecimal: 2))"
            label.text = Formatter.shared.appPriceFormat(string: "\(Float.removeNil(amount))")
        }
        ...
        setAmount(to: self.labelPeak, with: request.payment?.peak_amount)
        ...
        if self.discount == 0 {
            self.discount = request.payment?.discount ?? 0
        }
        ...
        if (request.paid == 1)
        {
         ...
         // dimpal 7 jan
        if request.payment_mode == .CARD && request.paid == 0
        {
            self.labelPaymentType.isHidden = true
            self.buttonChangePayment.isHidden = true
            self.buttonPayNow.isHidden = false
...
//        }
        ...
        if User.main.userType == Constants.string.usertype_company && request.use_wallet == 1 {
            buttonChangePayment.isHidden = true
            viewWaitingTimeAlert.isHidden = true
        }
    }
...
//        self.labelToPay.text = "\(String.removeNil(User.main.currency)) \(Formatter.shared.limit(string: "\(tipsAmount+payyable)", maximumDecimal: 2))"
        self.buttonTips.setTitle((tipsAmount>0 || self.isShowingRecipt) ? Formatter.shared.appPriceFormat(string: "\(tipsAmount)") : " \(Constants.string.addTips.localize()) ", for: .normal)
        
        print("&&&&&&&&&&&&&&&&&&&&", discount)
        if discount > 0 {
            self.labelTotal.text = Formatter.shared.appPriceFormat(string: "\(tipsAmount+total+discount)")
        } else {
            self.labelTotal.text = Formatter.shared.appPriceFormat(string: "\(tipsAmount+total)")
        }
        
        self.labelToPay.text = Formatter.shared.appPriceFormat(string: "\(tipsAmount+payyable)")
    }
    ...
    /*    if ischeck == false
        {
           ...
           // dimpal 7 jan
//        print("iswallet", request?.payment_mode)
            ...
            {
                if buttonPayNow.titleLabel?.text == Constants.string.Done.localize()
                {
                    if request?.use_wallet == 1 && request?.status == .completed {

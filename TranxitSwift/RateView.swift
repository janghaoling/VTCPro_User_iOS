...
class RateView: UIView {
    ...
    @IBOutlet weak var labelLuggageCapacity: UILabel!
    @IBOutlet weak var labelLuggageCapacityValue: UILabel!
...
extension RateView {
    ...
    private func setDesign() {
        ...
        Common.setFont(to: labelLuggageCapacity)
        Common.setFont(to: labelLuggageCapacityValue)
    ...
    private func localize() {
        ...
        self.labelLuggageCapacity.text = Constants.string.luggage_capacity.localize()
    }
    ...
    func set(values : Service?) {
      ...
      //  let fixed =  (self.service?.fixed?.floatValue())!
        
        self.labelBaseFare.text = Formatter.shared.appPriceFormat(string: "\(values?.fixed ?? 0)")
//        self.labelBaseFare.text = Formatter.shared.appPriceFormat(string: "\(values?.pricing?.base_price ?? 0)")
        self.labelFare.text = Formatter.shared.appPriceFormat(string: "\(values?.price ?? 0)")
        ...
        self.labelLuggageCapacityValue.text = "\(values?.luggage_capacity ?? 0)"
    }

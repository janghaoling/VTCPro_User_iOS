...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        let aps = notification[AnyHashable("aps")] as? NSDictionary
        let body = aps!["alert"] as? String        
            ...
            if notificationType == "chat" && notificationType != nil {
                if body!.contains("wants to call you!  Tap here to answer") {
                    NotificationCenter.default.post(name: Notification.Name(rawValue: "VoicePushRedirection"), object: nil)
                } else {
...
extension AppDelegate {
    ...
    // Stripe
    private func stripe(){
        if let stripeKey = User.main.stripeKey {

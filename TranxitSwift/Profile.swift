...
class Profile : JSONSerializable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        user_type = try? values.decode(String.self, forKey: .user_type)
        company_name = try? values.decode(String.self, forKey: .company_name)
        wallet_limit = try? values.decode(Double.self, forKey: .wallet_limit)
        allow_negative = try? values.decode(Int.self, forKey: .allow_negative)
    }

...
extension Interactor : PostInteractorOutputProtocol {
    ...
    case .changePassword, .resetPassword, .cancelRequest, .payNow, .locationServicePostDelete, .addPromocode, .logout, .postCards, .deleteCard, .userVerify, .rateProvider, .updateRequest, .phoneNubVerify, .updateRecurrent :
        ...
        case .checkdriverzone:
            self.presenter?.CheckDrivingZone(api: api, data: response)
            
        case .checkPoiPriceLogic:
            self.presenter?.CheckPoiPriceLogic(api: api, data: response)
            
        case .getServicePOIDisatnceInfo:
            self.presenter?.GetServicePOIDisatnceInfo(api: api, data: response)
            
        case .getServiceType:
            self.presenter?.GetServiceType(api: api, data: response)
            
            
        case .sendRequest:
            self.presenter?.sendRequest(api: api, data: response)
            
        case .sendStopRequest:
            self.presenter?.sendRequest(api: api, data: response)
        
        case .historyList, .upcomingList, .pastTripDetail, .upcomingTripDetail:
            self.presenter?.sendRequestArray(api: api, data: response)
            
//        case .upcomingTripDetail:
//            self.presenter?.sendRequest2Array(api: api, data: response)
//            

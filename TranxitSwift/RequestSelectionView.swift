...
class RequestSelectionView: UIView {
    ...
    var selectedCoupon : PromocodeEntity? { // Selected Promocode
        didSet
        {
            ...
            if let percentage = selectedCoupon?.percentage, let maxAmount = selectedCoupon?.max_amount
                
            {
                let price = self.service?.fixed
                let fare = price
                ...
                let fixed =  self.service?.fixed
                self.setEstimationFare(amount: fixed)
...
//            {
//                let text = "\(Constants.string.payment.localize()) : \("Wallet")"
           ...
           /* if isWalletChecked == true
            {
                if paymentType == .CASH
                {
                     ...
                     let text = "\(Constants.string.payment.localize()) : \("Wallet")"
                     self.labelPaymentMode.text = text
                   // self.labelPaymentMode.textColor = .secondary
                    
                  //  self.labelPaymentMode.startLocation = ((text.count)-(paymentString.count))
                 //   self.labelPaymentMode.length = paymentString.count
                    
                 //   self.labelPaymentMode.startLocation = ((text.count)-(6))
                 //   self.labelPaymentMode.length = 6
                }
            ...
            else
            {
                if paymentType == .CASH
                {
                    let text = "\(Constants.string.payment.localize()) : \("CASH")"
                    self.labelPaymentMode.text = text
                 //   self.labelPaymentMode.textColor = .secondary
...
extension RequestSelectionView {
    ...
    func setValues(values : Service) {
        ...
        let fixed = self.service?.fixed ?? 0.0
        
        if (User.main.allow_negative == 1 && User.main.userType == Constants.string.usertype_company && Float(User.main.wallet_limit ?? 0.0) > (fixed - User.main.wallet_balance!)) || User.main.userType == Constants.string.usertype_company {
            self.viewUseWallet.isHidden = false
        } else {
...
//        self.labelWalletBalance.text = "\(String.removeNil(User.main.currency)) \(Formatter.shared.limit(string: "\(Float.removeNil(self.service?.pricing?.wallet_balance))", maximumDecimal: 2))"
        self.labelWalletBalance.text = Formatter.shared.appPriceFormat(string: "\(Float.removeNil(self.service?.pricing?.wallet_balance))")
    }
...
//        self.labelEstimationFare.text = "\(User.main.currency ?? .Empty) \(Formatter.shared.limit(string: "\(amount ?? 0)", maximumDecimal: 2))"
        self.labelEstimationFare.text = Formatter.shared.appPriceFormat(string: "\(amount ?? 0)")
    }

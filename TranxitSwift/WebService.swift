...
class Webservice : PostWebServiceProtocol {
    ...
    var completion : ((CustomError?, Data?)->())?
     ...
     var str_errormessage : NSDictionary?
    ...
    func retrieve(api: Base,url : String?, data: Data?, imageData: [String:Data]?, paramters: [String : Any]?, type: HttpType, completion : ((CustomError?, Data?)->())?) {
     ...
     /* if api.rawValue  == "/api/user/request/check"
        {
            return
        }
        */
    ...
    fileprivate func send(_ response: (DataResponse<Any>)?) {
       ...
       // let Dicdata = response!.response?.value(forKey: "result")as! NSDictionary
        ...
        if response.response?.statusCode == StatusCode.unAuthorized.rawValue  { // Validation For UnAuthorized Login
            ...
            if let error = response.data?.getDecodedObject(from: ErrorLogger.self)
            {  // Retriving Error message from Server
                
                message = error.error
            }
            self.completion?(CustomError(description: ErrorMessage.list.serverError.localize(), code: (response.response?.statusCode) ?? StatusCode.unAuthorized.rawValue), nil)
            forceLogout(with: message) // Force Logout user by clearing all cache
            
            
        }
            
        else if apiType.rawValue == "/api/user/send/requests" && (self.str_errormessage!["error"]  as? String) != nil
        {
            if self.str_errormessage?.value(forKey: "error")as! String  == "No Drivers Found"
            {
                
           
               print("Ride now Test Hemant \(response.result.value!))")
            if response.response?.statusCode == StatusCode.success.rawValue
            {  //  Validation for Error Log
        ...
        // 25/11/2019 change Ride now 
           
       /*  else if apiType.rawValue == "/api/user/send/requests"
        {
             if response.response?.statusCode == StatusCode.success.rawValue
             {  //  Validation for Error Log
    ...
    func send(api: Base, url appendingUrl : String?, data: Data?, parameters : [String : Any]?, type : HttpType) {
        ...
        print("access_token =>\(WebConstants.string.bearer+String.removeNil(User.main.accessToken))");
        ...
        Alamofire.request(urlRequest!).validate(statusCode: StatusCode.success.rawValue..<StatusCode.multipleResponse.rawValue).responseJSON { (response) in
            ...
            case .success(let val):
                self.str_errormessage = val  as? NSDictionary

...
class SignUpUserTableViewController: UITableViewController {
    ...
    @IBOutlet weak var ivCheckedAgreed: UIImageView!
    @IBOutlet weak var lbTermsConditions: UILabel!
    @IBOutlet weak var btnAgree: UIButton!
    var terms_conditions_url = "http://167.71.57.246/terms"
...
//    private var accountKit : AKFAccountKit?
    ...
    private var isSMSVerified = false
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Webservice().retrieve(api: .terms_conditions, url: nil, data:nil, imageData: nil, paramters: nil, type: .GET) { (error, data) in
            struct TC : JSONSerializable {
                var url : String?
            }
            let responseObject = data?.getDecodedObject(from: TC.self)
            self.terms_conditions_url = responseObject?.url ?? "http://167.71.57.246/terms"
        }
    }
    ...
    @IBAction func onClickAgree(_ sender: Any) {
        ...
        self.ivCheckedAgreed.image = self.btnAgree.isSelected ? UIImage(named: "check") : UIImage(named: "check-box-empty")
    }
    
    @IBAction func onClickTermLb(_ sender: Any) {
        let url = URL.init(string:self.terms_conditions_url)
...
extension SignUpUserTableViewController {
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case 5...7:
            return 1
        case 8:
            return self.viewReferCode.isHidden ? 0 : 80
        case 9:
            return 1
        case 10:
            return 60
    ...
    private func localize(){
        ...
        self.lbTermsConditions.text = Constants.string.theTermsConditions.localize()
...
//    }
    ...
    private func getSMS() {
        if let getSMSVC = Router.user.instantiateViewController(withIdentifier: Storyboard.Ids.GetSMSViewController) as? GetSMSViewController {
            getSMSVC.phone = phoneNumber.text!
            getSMSVC.email = emailtext.text!
            getSMSVC.country_Code = countryCode ?? "FR"
            getSMSVC.onCompleteVerified = { code, phone in
            ...
            self.navigationController?.pushViewController(getSMSVC, animated: true)
            return
        }
    }
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if textField == countryText {
            ...
            vc.searchCountryCode = { code in
                ...
                for eachCountry in country {
                    if code == eachCountry.code {
                        self.isSMSVerified = self.isSMSVerified && self.countryCode == code
                        self.countryCode = code
                        self.countryText.text = eachCountry.dial_code
                        let myImage = UIImage(named: "CountryPicker.bundle/\(eachCountry.code).png")
                        self.countryImageView.image = myImage

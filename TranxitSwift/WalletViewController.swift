...
class WalletViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.addAmount.setTitle(Constants.string.ADDAMT.localize(), for: .normal)
    }
...
//        self.textFieldAmount.placeholder = String.removeNil(User.main.currency)+" "+"\(0)"
        self.textFieldAmount.placeholder = Formatter.shared.appPriceFormat(string:"\(0)")
...
//            button.setTitle(String.removeNil(String.removeNil(User.main.currency)+" \(button.tag)"), for: .normal)
            button.setTitle(Formatter.shared.appPriceFormat(string:"\(button.tag)"), for: .normal)
        }
    ...
    private func setWalletBalance() {
        DispatchQueue.main.async {
            self.labelBalance.text = Formatter.shared.appPriceFormat(string: "\(User.main.wallet_balance ?? 0)")
